...
[MCP](https://modelcontextprotocol.io) server exposing the UnsolicitedAdvice SDK as
two agent tools — `unsolicited-advice_list` and `unsolicited-advice_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unsolicited-advice-mcp)
make build

# 2. Provide credentials via the environment
export UNSOLICITED_ADVICE_APIKEY=sk_live_xxx
...
```jsonc
// unsolicited-advice_list: first page of records
{ "entity": "advice" }
{ "entity": "advice", "query": { } }

// unsolicited-advice_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unsolicited-advice-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user unsolicited-advice \
     -- "$PWD"/dist/*/unsolicited-advice-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `unsolicited-advice_list` and `unsolicited-advice_load` tools now appear
   in new sessions. Ask the agent to *"list advice using unsolicited-advice"*
   and it calls `unsolicited-advice_list` with `{"entity":"advice"}`.
...
```sh
export UNSOLICITED_ADVICE_APIKEY=sk_live_xxx            # API key
export UNSOLICITED_ADVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `unsolicited-advice_list` tool
...
### Call the `unsolicited-advice_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `unsolicited-advice_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `unsolicited-advice_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UNSOLICITED_ADVICE_APIKEY` | API key sent with every request. |
| `UNSOLICITED_ADVICE_BASE` | Optional override of the API base URL. |
