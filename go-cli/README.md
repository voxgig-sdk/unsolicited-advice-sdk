# unsolicited-advice-cli

boru-driven command-line client **and** interactive REPL for the UnsolicitedAdvice
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/unsolicited-advice-cli)
make build

# 2. See usage (words, entities, env vars)
./unsolicited-advice-cli --help

# 3. Provide credentials once, via the environment
export UNSOLICITED_ADVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./unsolicited-advice-cli list advice
./unsolicited-advice-cli load 1 advice            # {id:1} shorthand
./unsolicited-advice-cli load '{id:1}' advice       # explicit match map

# 5. Override the API base URL for a single call
UNSOLICITED_ADVICE_BASE=https://api.example.com ./unsolicited-advice-cli list advice

# 6. No arguments -> interactive REPL
./unsolicited-advice-cli
unsolicited-advice> list advice
unsolicited-advice> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/unsolicited-advice-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/unsolicited-advice-cli list advice
   ```
...
```sh
./unsolicited-advice-cli load 1 advice          # scalar shorthand for {id:1}
./unsolicited-advice-cli load '{id:1}' advice     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `unsolicited-advice>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `advice`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/unsolicited-advice-cli`. |
...
(`./unsolicited-advice-cli <expr>`) and interactively (the REPL), and expressions compose the
