// Command UnsolicitedAdvice-cli is a CLI + REPL for the UnsolicitedAdvice SDK,
...
const prompt = "unsolicited-advice"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "advice"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UNSOLICITED_ADVICE_APIKEY (api key), UNSOLICITED_ADVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UNSOLICITED_ADVICE_APIKEY carries the API key and
	// UNSOLICITED_ADVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UNSOLICITED_ADVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UNSOLICITED_ADVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUnsolicitedAdviceSDK(opts)
