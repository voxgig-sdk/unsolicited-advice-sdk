...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.UNSOLICITEDADVICE_TEST_LIVE ||
    'TRUE' === process.env.UNSOLICITEDADVICE_TEST_OVERRIDE
  ) {
  ...
  m.UNSOLICITEDADVICE_TEST_EXPLAIN = process.env.UNSOLICITEDADVICE_TEST_EXPLAIN || m.UNSOLICITEDADVICE_TEST_EXPLAIN
