...
describe('AdviceDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new UnsolicitedAdviceSDK({
  ...
  test('direct-list-advice', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-advice', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['UNSOLICITED_ADVICE_TEST_ADVICE_ENTID']
