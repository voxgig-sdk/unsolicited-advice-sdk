...
//     (\`new UnsolicitedAdviceSDK(...)\` / \`UnsolicitedAdviceSDK.test(...)\` become
//     \`UnsolicitedAdviceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UnsolicitedAdviceSDK'
...
const TEST_SEED = {"entity":{"advice":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UnsolicitedAdviceSDK(...)\` and any
// \`UnsolicitedAdviceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UnsolicitedAdviceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UnsolicitedAdviceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
