...
require_relative 'UnsolicitedAdvice_types'
...
class UnsolicitedAdviceSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UnsolicitedAdviceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UnsolicitedAdviceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UnsolicitedAdviceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => UnsolicitedAdviceError.new(
        "#{op}_allow",
        "UnsolicitedAdviceSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue UnsolicitedAdviceError => err
    ...
    if fetched.is_a?(Hash)
      status = UnsolicitedAdviceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UnsolicitedAdviceError.new(
        "graphql_error", "UnsolicitedAdviceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UnsolicitedAdviceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
