...
module UnsolicitedAdviceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UnsolicitedAdviceResponse.new({})
    ...
    if fetch_err
      ...
      response = UnsolicitedAdviceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UnsolicitedAdviceResponse.new(fetched)
    else
