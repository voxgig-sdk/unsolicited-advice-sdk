# UnsolicitedAdvice SDK Advice entity
...
class AdviceEntity
  ...
  def data_set(args)
    if args
      @_data = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UnsolicitedAdviceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [AdviceListMatch, Hash, nil] match filter (any subset of
  #   Advice fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
