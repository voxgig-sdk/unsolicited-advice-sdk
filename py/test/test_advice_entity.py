...
class TestAdviceEntity:
    ...
    def test_should_stream(self):
        ...
        base = UnsolicitedAdviceSDK.test(seed, None)
        seen = list(base.Advice(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = UnsolicitedAdviceSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Advice(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UNSOLICITED_ADVICE_TEST_ADVICE_ENTID JSON to run live")
        client = setup["client"]
...
def _advice_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("UNSOLICITED_ADVICE_TEST_ADVICE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("UNSOLICITED_ADVICE_TEST_LIVE") == "TRUE":
        ...
        client = UnsolicitedAdviceSDK(helpers.to_map(merged_opts))

    _live = env.get("UNSOLICITED_ADVICE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("UNSOLICITED_ADVICE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
