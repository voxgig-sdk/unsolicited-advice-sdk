# UnsolicitedAdvice SDK test runner
...
class UnsolicitedAdviceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UnsolicitedAdviceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UnsolicitedAdviceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UnsolicitedAdviceTestRunner.getenv("UNSOLICITED_ADVICE_TEST_LIVE")
        override = UnsolicitedAdviceTestRunner.getenv("UNSOLICITED_ADVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UnsolicitedAdviceTestRunner.getenv(key)
        ...
        explain = UnsolicitedAdviceTestRunner.getenv("UNSOLICITED_ADVICE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["UNSOLICITED_ADVICE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if UnsolicitedAdviceTestRunner._test_control is not None:
            return UnsolicitedAdviceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UnsolicitedAdviceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UnsolicitedAdviceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UnsolicitedAdviceTestRunner.load_test_control()
...
def load_env_local():
    UnsolicitedAdviceTestRunner.load_env_local()


def env_override(m):
    return UnsolicitedAdviceTestRunner.env_override(m)


def entity_data(v):
    return UnsolicitedAdviceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UnsolicitedAdviceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UnsolicitedAdviceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UnsolicitedAdviceTestRunner.load_test_control()


def live_client_options():
    return UnsolicitedAdviceTestRunner.live_client_options()


def live_delay_ms():
    return UnsolicitedAdviceTestRunner.live_delay_ms()
