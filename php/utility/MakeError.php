...
class UnsolicitedAdviceMakeError
{
    public static function call(?UnsolicitedAdviceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UnsolicitedAdviceContext([], null);
        }
        $op = $ctx->op ?? new UnsolicitedAdviceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UnsolicitedAdviceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UnsolicitedAdviceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UnsolicitedAdviceError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof UnsolicitedAdviceError) {
