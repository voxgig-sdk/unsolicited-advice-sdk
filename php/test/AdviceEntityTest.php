...
class AdviceEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = UnsolicitedAdviceSDK::test($seed, null);
        $seen = iterator_to_array($base->Advice(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = UnsolicitedAdviceConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = UnsolicitedAdviceSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Advice(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set UNSOLICITED_ADVICE_TEST_ADVICE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function advice_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("UNSOLICITED_ADVICE_TEST_ADVICE_ENTID");
    ...
    if ($env["UNSOLICITED_ADVICE_TEST_LIVE"] === "TRUE") {
        ...
        $client = new UnsolicitedAdviceSDK(Helpers::to_map($merged_opts));
    }
