...
//    only - a typed UnsolicitedAdviceContext there would make every ctx
...
final class UnsolicitedAdviceOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UnsolicitedAdviceContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new UnsolicitedAdviceSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new UnsolicitedAdviceResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new UnsolicitedAdviceError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new UnsolicitedAdviceResponse($clean['response']);
    ...
    public static function err_from_map($m): ?UnsolicitedAdviceError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UnsolicitedAdviceError(is_string($code) ? $code : '', $msg);
    }
