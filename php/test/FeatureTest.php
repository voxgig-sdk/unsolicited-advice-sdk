...
// UnsolicitedAdvice SDK feature test
//
...
class FtCtrl extends UnsolicitedAdviceControl
{
...
class FtClient
{
    ...
    private UnsolicitedAdviceUtility $utility;

    public function __construct(UnsolicitedAdviceUtility $utility, array $options)
    {
    ...
    public function get_utility(): UnsolicitedAdviceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UnsolicitedAdviceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UnsolicitedAdviceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UnsolicitedAdviceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UnsolicitedAdviceConfig::make_feature($name);
    ...
    private static function buildUrl(UnsolicitedAdviceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UnsolicitedAdviceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UnsolicitedAdviceResponse($fetched);
        }
    ...
    private function fail(UnsolicitedAdviceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UnsolicitedAdviceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
