...
// UnsolicitedAdvice SDK pipeline test
//
...
{
    private static function utility(): UnsolicitedAdviceUtility
    {
        return new UnsolicitedAdviceUtility();
    }
    ...
    private static function ctx(array $over = []): UnsolicitedAdviceContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UnsolicitedAdviceContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UnsolicitedAdviceError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UnsolicitedAdviceMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UnsolicitedAdviceMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UnsolicitedAdviceMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnsolicitedAdviceSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UnsolicitedAdviceMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UnsolicitedAdviceResponse([]),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        [, $err] = UnsolicitedAdviceMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec([]),
            'response' => null,
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        [, $err] = UnsolicitedAdviceMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec(['step' => 's']),
            'response' => new UnsolicitedAdviceResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        [, $err] = UnsolicitedAdviceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec(['step' => 's']),
            'response' => new UnsolicitedAdviceResponse(self::resp(200, ['v' => 1])),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        [, $err] = UnsolicitedAdviceMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec(['step' => 's']),
            'response' => new UnsolicitedAdviceResponse(self::resp(200, ['v' => 2])),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UnsolicitedAdviceMakeResponse::call($ctx);
    ...
    {
        $preset = new UnsolicitedAdviceResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec([]),
            'response' => new UnsolicitedAdviceResponse([]),
            'result' => new UnsolicitedAdviceResult([]),
        ...
        [$got, $err] = UnsolicitedAdviceMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UnsolicitedAdviceResult([])]);
        [, $err] = UnsolicitedAdviceMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UnsolicitedAdviceSpec([]), 'result' => null]);
        [, $err] = UnsolicitedAdviceMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnsolicitedAdviceSpec(['step' => 's']),
            'result' => new UnsolicitedAdviceResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnsolicitedAdviceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UnsolicitedAdviceSpec(['step' => 's']),
            'result' => new UnsolicitedAdviceResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UnsolicitedAdviceOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UnsolicitedAdviceMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UnsolicitedAdviceResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UnsolicitedAdviceSpec([]),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UnsolicitedAdviceMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UnsolicitedAdviceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UnsolicitedAdviceError('boom', 'boom');
        $utility->fetcher = function (UnsolicitedAdviceContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnsolicitedAdviceSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UnsolicitedAdviceMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UnsolicitedAdviceContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UnsolicitedAdviceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UnsolicitedAdviceResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        UnsolicitedAdviceMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UnsolicitedAdviceContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UnsolicitedAdviceSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UnsolicitedAdviceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UnsolicitedAdviceResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UnsolicitedAdviceSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UnsolicitedAdviceMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UnsolicitedAdviceMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UnsolicitedAdviceMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UnsolicitedAdviceResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UnsolicitedAdviceDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UnsolicitedAdviceResult(['ok' => false])]);
        $this->expectException(UnsolicitedAdviceError::class);
        UnsolicitedAdviceDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UnsolicitedAdviceResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UnsolicitedAdviceMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        UnsolicitedAdviceMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UnsolicitedAdviceBaseFeature();
        $b = new UnsolicitedAdviceBaseFeature();
        UnsolicitedAdviceFeatureAdd::call($ctx, $a);
        UnsolicitedAdviceFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UnsolicitedAdviceBaseFeature
    {
        $f = new UnsolicitedAdviceBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UnsolicitedAdviceFeatureAdd::call($ctx, self::named_feature('a'));
        UnsolicitedAdviceFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        UnsolicitedAdviceFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        UnsolicitedAdviceFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        UnsolicitedAdviceFeatureAdd::call($ctx, $replace);
        ...
        UnsolicitedAdviceFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UnsolicitedAdviceContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UnsolicitedAdviceSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UnsolicitedAdvicePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        UnsolicitedAdvicePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UnsolicitedAdviceResponse(['headers' => null]),
            'result' => new UnsolicitedAdviceResult([]),
        ]);
        UnsolicitedAdviceResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        UnsolicitedAdviceResultBody::call($ctx);
