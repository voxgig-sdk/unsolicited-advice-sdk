...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("UNSOLICITED_ADVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("UNSOLICITED_ADVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("UNSOLICITED_ADVICE_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.UnsolicitedAdviceSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.UnsolicitedAdviceSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.UnsolicitedAdviceError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.UnsolicitedAdviceSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.UnsolicitedAdviceError{Msg: msg, Code: code}
}
