...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("UNSOLICITEDADVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("UNSOLICITEDADVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("UNSOLICITEDADVICE_TEST_EXPLAIN"); explain != "" {
