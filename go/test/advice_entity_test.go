...
func TestAdviceEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Advice(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Advice(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		adviceRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.advice")))
		...
		adviceRef01Match := map[string]any{}

		adviceRef01ListResult, err := adviceRef01Ent.List(adviceRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, adviceRef01ListOk := adviceRef01ListResult.([]any)
		if !adviceRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", adviceRef01ListResult)
		}

		// LOAD
		adviceRef01MatchDt0 := map[string]any{
			"id": adviceRef01Data["id"],
		}
		...
		if adviceRef01DataDt0LoadResult["id"] != adviceRef01Data["id"] {
