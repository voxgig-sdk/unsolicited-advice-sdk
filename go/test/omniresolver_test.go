...
import (
	...
	omni "github.com/voxgig-sdk/unsolicited-advice-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UnsolicitedAdviceError); is {
				out := map[string]any{
					"name":    "UnsolicitedAdviceError",
...
func omniCtx(arg any, client *sdk.UnsolicitedAdviceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UnsolicitedAdviceSDK); is {
			client = live
