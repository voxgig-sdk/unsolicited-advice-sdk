...
// UnsolicitedAdvice SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = UnsolicitedAdviceOmni::makeStructRunner(
    ...
    {
        return UnsolicitedAdviceOmni::objmodel(UnsolicitedAdviceOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return UnsolicitedAdviceOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return UnsolicitedAdviceOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(UnsolicitedAdviceOmni::structfix($now, Struct::undef()))
                        !== json_encode(UnsolicitedAdviceOmni::structfix($val, Struct::undef()))) {
