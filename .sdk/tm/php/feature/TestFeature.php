...
// UnsolicitedAdvice SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class UnsolicitedAdviceTestFeature extends UnsolicitedAdviceBaseFeature
{
    ...
    public function init(UnsolicitedAdviceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (UnsolicitedAdviceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = UnsolicitedAdviceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (UnsolicitedAdviceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(UnsolicitedAdviceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = UnsolicitedAdviceParam::call($ctx, $k);
