...
// UnsolicitedAdvice SDK retry feature
...
class UnsolicitedAdviceRetryFeature extends UnsolicitedAdviceBaseFeature
{
    ...
    public function init(UnsolicitedAdviceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UnsolicitedAdviceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UnsolicitedAdviceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UnsolicitedAdviceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
