...
module UnsolicitedAdviceOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if UnsolicitedAdviceOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = UnsolicitedAdviceOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| UnsolicitedAdviceOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || UnsolicitedAdviceOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = UnsolicitedAdviceSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = UnsolicitedAdviceError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { UnsolicitedAdviceOmni.errify(err) }
